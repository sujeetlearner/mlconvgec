...
mkdir -p "$output_dir"
! python3 "$SCRIPTS_DIR/apply_bpe.py" -c models/bpe_model/train.bpe.model < $input_file > $output_dir/input.bpe.txt

# running fairseq on the test data
CUDA_VISIBLE_DEVICES=$device python3 "$FAIRSEQPY/interactive.py"  --path $models --beam $beam --nbest $beam --source-lang src --target-lang trg  processed/bin  < $output_dir/input.bpe.txt > $output_dir/output.bpe.nbest.txt
