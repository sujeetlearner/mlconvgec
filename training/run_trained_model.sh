...
CUDA_VISIBLE_DEVICES=$device python3 $FAIRSEQPY/interactive.py --no-progress-bar --path $models --beam $beam --nbest $beam --source-lang src --target-lang trg  processed/bin  < $output_dir/input.bpe.txt > $output_dir/output.bpe.nbest.txt
